...
public class SkillController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAll() =>
        Ok(
            (await _repo.GetAllAsync()).Select(s => new
            {
                s.Id,
                s.Nome,
                _links = new { self = $"/api/v1/skills/{s.Id}" },
            })
        );
    ...
    public async Task<IActionResult> GetById(int id)
    {
        ...
        return Ok(
            new
            {
                ...
                item.Nome,
                item.Descricao,
                _links = new { self = $"/api/v1/skills/{item.Id}" },
            }
        );
    }
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, Skill model)
    {
