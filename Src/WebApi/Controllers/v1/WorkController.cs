...
public class WorkController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAll() =>
        Ok(
            (await _repo.GetAllAsync()).Select(w => new
            {
                w.Id,
                w.Titulo,
                _links = new { self = $"/api/v1/works/{w.Id}" },
            })
        );
    ...
    public async Task<IActionResult> GetById(int id)
    {
        ...
        return Ok(
            new
            {
                ...
                it.Titulo,
                it.Descricao,
                _links = new { self = $"/api/v1/works/{it.Id}" },
            }
        );
    }

    [HttpPost]
    public async Task<IActionResult> Create(Work m)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, Work m)
    {
